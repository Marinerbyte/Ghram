...
def groq_ai_engine(user, prompt):
    ...
    Enforces personality and constraints via System Prompt.
    """
    ...
    if BOT_STATE["mode"] == "ar":
        sys_prompt = f"""You are {my_name}. You are NOT an AI.
...
3. Mild sarcasm only when it fits.
4. Use emojis only when appropriate.
   Emoji pool: 🙂 😄 😅 😉 🙃 😌 🤝 ☕ ✨
5. Max 15 words per message.
6. No insults or personal attacks.
7. If teased, reply smartly and friendly.
"""
    else:
        sys_prompt = f"""You are {my_name}. You are NOT an AI.
You are a funny, friendly user in a chatroom.

RULES:
1. Speak English naturally.
2. Light humor, no toxicity.
3. Be witty, not rude.
4. Emojis only when they fit.
5. Max 15 words per message.
"""
